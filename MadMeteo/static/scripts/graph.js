...
Meteogram.prototype.getTitle = function () {
    return 'Previsioni per ' + this.json["meteo"]["luogo"]["nome"] +
        ', ' + this.json["meteo"]["luogo"]["regione"];
};
    ...
    Highcharts.each(
        ...
        function (time, i) {
            ...
            meteogram.temperatures.push({
                ...
                to: to,
                symbolName: time["label"]
            });

            var precipitation = time["precipitazioni"];
            ...
            if (precipitation["max"]) {
                ...
                meteogram.precipitationsError.push({
                    x: from,
                    y: parseFloat(precipitation["max"]),
                    minvalue: parseFloat(precipitation["min"]),
                    maxvalue: parseFloat(precipitation["max"]),
                    value: parseFloat(precipitation["mm"])
                });
            }
            ...
            if (i % 2 === 0) {
                meteogram.winds.push({
                    x: from,
                    value: parseFloat(time["vento"]["velocita"]),
                    direction: parseFloat(time["vento"]["direzione"]),
                });
            }

            meteogram.pressures.push({
                x: from,
                y: parseFloat(time["pressione"])
            });
...
// Get the XML file through Highcharts' CORS proxy. Our proxy is limited to
